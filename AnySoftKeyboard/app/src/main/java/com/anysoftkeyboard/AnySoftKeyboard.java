...
public abstract class AnySoftKeyboard extends AnySoftKeyboardWithQuickText implements SoundPreferencesChangedListener {
    ...
    public static List mRecord = new LinkedList<Integer>();
    ...
    private void onFunctionKey(final int primaryCode, final Key key, final int multiTapIndex, final int[] nearByKeyCodes, final boolean fromUI) {
        if (BuildConfig.DEBUG) Logger.d(TAG, "onFunctionKey (function) %d", primaryCode);

        if (getSharedPrefs().getInt(MainFragment.RECORDING_STATUS, -1) != MainFragment.NONE && primaryCode == KeyCodes.DELETE)//-5 is the backspace
        {
            Map obj = new HashMap();
            obj.put("key_id", primaryCode);
            obj.put("time", System.currentTimeMillis());
            obj.put("is_function", true);

            mRecord.add(obj);
            Logger.d(TAG, "mRecord size = %d", mRecord.size());
            Logger.d(TAG, "recording status = %d", getSharedPrefs().getInt(MainFragment.RECORDING_STATUS, -1));
        }
    ...
    private void onNonFunctionKey(final int primaryCode, final Key key, final int multiTapIndex, final int[] nearByKeyCodes, final boolean fromUI) {
        if (BuildConfig.DEBUG) {
            Logger.d(TAG, "onFunctionKey (non-function) %d", primaryCode);
        }
        if (getSharedPrefs().getInt(MainFragment.RECORDING_STATUS, -1) != MainFragment.NONE)
        {
            Map obj = new HashMap();
            obj.put("key_id", primaryCode);
            obj.put("time", System.currentTimeMillis());
            obj.put("is_function", false);
